...
@Component({
    selector: 'app-jefes-aceptados',
    templateUrl: './jefes-aceptados.component.html',
    styleUrls: ['./jefes-aceptados.component.css'],
})
export class JefesAceptadosComponent implements OnInit {
    Alumno: Alumno[] = [];
    ...
    ngOnInit() {
        ...
        this.getAlumnosAceptados(this.userToken);
    }

    getAlumnosAceptados(token: string) {
        this.jefesService.getAlumnosAceptados(token).subscribe(
            (response: any[]) => {
    ...
    rechazarAlumno(id: string, nombreCom: string): void {
        ...
        //                 this.jefesService.rechazarAlumnoComi(id, motivoRechazo).subscribe(
        //                     (response) => {
    ...
    aceptarAlumno(id: string, nombreCom: string): void {
        ...
        // }).then((result) => {
        //     if (result.isConfirmed) {
        //         this.jefesService.aceptarAlumnoComi(id).subscribe(
        //             (response) => {
    ...
    buscarPorMatricula(event: Event) {
        ...
        if (matricula === '') {
            this.getAlumnosAceptados(this.userToken); // Si el campo está vacío, mostrar todos los alumnos
        } else {
